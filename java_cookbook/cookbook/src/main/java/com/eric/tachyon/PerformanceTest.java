...
public class PerformanceTest {
	...
	public static void createFiles() throws IOException {
		...
		for (int k = 0; k < sFiles; k++) {
			if(sMtc.exist(new TachyonURI(sFileName + (k + sBaseFileNumber)))){
				sMtc.delete(new TachyonURI(sFileName + (k + sBaseFileNumber)), true);
			}
	...
	public static class GeneralWorker extends Worker {
		...
		public GeneralWorker(int id, int left, int right, ByteBuffer buf, boolean oneToMany, boolean memoryOnly,
				String msg) {
...
//			tConf.set("dfs.client.read.shortcircuit.skip.checksum", "true");
		...
		public void io() throws IOException {
			...
			if (mWrite) {
				for (int times = mLeft; times < mRight; times++) {
					...
					OutputStream os = mHdfsFs.create(new Path(filePath),true);
					for (int k = 0; k < sBlocksPerFile; k++) {
						mBuf.putInt(0, k + mWorkerId);
						os.write(mBuf.array());
					}
	...
	private static void memoryCopyTest(boolean write, boolean memoryOnly) {
		...
		for (int thread = 0; thread < sThreads; thread++) {
			workerThreads[thread] = new GeneralWorker(thread, t * thread, t * (thread + 1), bufs[thread], write,
					memoryOnly, msg);
		}
		...
		LOG.info(result + " Mb/sec. " + sResultPrefix + "Entire " + msg + " Test : " + " Took " + takenTimeMs
	...
	private static void TachyonTest(boolean write) throws IOException {
		...
		for (int thread = 0; thread < sThreads; thread++) {
			if (write) {
				workerThreads[thread] = new TachyonWriterWorker(thread, t * thread, t * (thread + 1), bufs[thread]);
			} else {
				workerThreads[thread] = new TachyonReadWorker(thread, t * thread, t * (thread + 1), bufs[thread]);
			}
		}
		...
		LOG.info(result + " Mb/sec. " + sResultPrefix + "Entire " + (write ? "Write " : "Read ") + " Took "
	...
	public static void main(String[] args) throws IOException {
		...
		// + "<MasterIp> <FileNamePrefix> <WriteBlockSizeInBytes>
		// <BlocksPerFile> "
		...
		System.out.println("1: Files Write Test\n" + "2: Files Read Test\n" + "3: RamFile Write Test \n"
				+ "4: RamFile Read Test \n" + "5: ByteBuffer Write Test \n" + "6: ByteBuffer Read Test \n"
				+ "0: Quit \n");
		sMasterAddress = new TachyonURI("tachyon://192.168.116.128:19998");
		sFileName = "/tmphdfs/performance";
		sBlockSizeBytes = 1024;
		sBlocksPerFile = 100;
		sDebugMode = false;
		sThreads = 5;
		sFiles = 1000 * sThreads;
		sBaseFileNumber = 100;

		while (true) {
			System.out.print("请输入测试类型：");
